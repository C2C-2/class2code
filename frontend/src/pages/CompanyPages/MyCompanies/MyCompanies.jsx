...
function MyCompanies() {
  ...
  const { loading, error, data } = useQuery(
    filterType ? GET_USER_COMPANIES_Filter : GET_USER_COMPANIES,
    {
      variables: { userId: user_id, filterType: filterType, desc: desc },
    }
  );
  const {
  ...
  const handleSearch = () => {
    // Call the search query when the search button is clicked
    setSearchWord(searchWord.trim()); // Trim any whitespace from the search word
  };
  return (
